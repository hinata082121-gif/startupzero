...
export const BannerAd = () => {
  ...
  return (
    <div className="fixed inset-x-0 bottom-28 z-20 border-t border-slate-300 bg-white/95 px-4 py-2 shadow-lg backdrop-blur lg:bottom-0">
      <div className="mx-auto flex max-w-7xl items-center justify-center gap-4 text-xs text-slate-600">
        <span className="font-semibold uppercase tracking-wide text-slate-500">{t("ads.advertisement")}</span>
      </div>
    </div>
  );
};
...
export const SidebarAd = () => {
  ...
  return (
    <aside className="hidden rounded-lg border border-slate-200 bg-white p-4 shadow-sm xl:block">
      <div className="flex h-[250px] items-center justify-center rounded-md bg-slate-100 p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{t("ads.advertisement")}</p>
      </div>
    </aside>
  );
};
